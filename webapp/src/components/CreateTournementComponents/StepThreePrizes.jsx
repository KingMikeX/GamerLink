...
useEffect(() => {
  ...
  return (
    <div className="bg-[#121428] mx-auto p-6 rounded-lg max-w-3xl">
      ...
      {prizeType !== 'none' && (
        <>
          <h3 className="mb-4 font-medium text-sm uppercase">Preise</h3>
          <div className="space-y-4 mb-6">
            {(formData.prizes || []).map((prize, index) => (
              <div key={prize.id} className="bg-[#131320] p-4 rounded-lg">
                <div className="flex items-center mb-2">
                  <span className="mr-3 font-bold text-purple-500 text-xl">{index + 1}.</span>
                  <input
                    type="text"
